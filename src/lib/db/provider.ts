...
export function assertVercelDatabase(operation: string): void {
  if (isVercelRuntime() && !isSupabaseConfigured()) {
    throw new Error(
      `${operation}: Vercel requires Supabase. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in Vercel Environment Variables, run supabase/migrations/001_initial_schema.sql, create a public "media" storage bucket, then npm run migrate-to-supabase.`
    );
  }
}

/** @deprecated Use assertVercelDatabase */
export function assertWritableDatabase(operation: string): void {
  assertVercelDatabase(operation);
}
